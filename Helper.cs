...
public class Helper
{
   public static string? Ask(string question, bool isRequired = false, string validationMsg = "You can not let this place empty.")
    {
        string? response;
        do
        {
    ...
    public static int AskNumber(string question, string validationMsg = "You should enter a number")
    {
        while (true)
        {
    ...
    public static int AskOption(string[] options, string? question = null, string? cancelOption = null)
    {
        if (options.Length == 0)
        {
            throw new ArgumentException($"{nameof(options)} must be options inside.", nameof(options));
        }
        ...
        while (true)
        {
            var inputResponse = AskNumber($"Your Choise ({optionsStartFrom}-{options.Length})");
            if (inputResponse >= optionsStartFrom && inputResponse <= options.Length)
            {
                return inputResponse;
            }
    
            ShowErrorMsg("Wrong choise");
        }
    }

    public static string AskPassword(string question, string validationMsg = "You can not let this place empty.")
    {
        var password = "";
        do
        {
...
public class ConsoleMenu(string title, bool isRoot = false)
{
    ...
    public void Show()
    {
        while (true)
        {
            ...
            Console.WriteLine();
            var inputMenuNumber = Helper.AskOption(_options.Select(x => x.Title).ToArray(), cancelOption: isRoot ? "Exit" : "Upper Menu");
